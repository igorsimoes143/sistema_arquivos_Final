...
def ping(uri2):
    sistema_monitor=Pyro5.client.Proxy(uri2)
    ...
    while True:
        ...
        # 3. Pausa a execução por 10 segundos
...
class sist_arq:
    ...
    def enviar_dados_node(self, chunk, nome_arq, idx):
        ...
        try:
            ...
            # dados_lidos = caminho_arquivo.read_bytes()
            # print(f"Verificação: Conteúdo lido do arquivo = {dados_lidos}")
    ...
    def ler_dados_node(self,nome_arq, indice):
        ...
        # print(f"Tentando ler o arquivo: {caminho_completo_alvo}")
    ...
    def remover_dados_node(self, nome_arq, indice):
        ...
        if caminho_completo_alvo.is_file():
            try:
                # O método .unlink() deleta o arquivo
...
ns.register("node3", uri)
print("uri do objeto remoto: ", uri)
uri2 = ns.lookup("monitor")
...
try:
    heartbeat_thread = threading.Thread(
        target=ping, 
        args=(uri2,), # Envia a cada 30 segundos
        daemon=True
    )
